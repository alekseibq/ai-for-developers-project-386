...
@pytest.fixture
def use_case(mock_repo) -> ListAllMeetingTypeUseCase:
    return ListAllMeetingTypeUseCase(repo=mock_repo)


class TestListAllMeetingType:
    async def test_returns_list_of_types(self, use_case: ListAllMeetingTypeUseCase, mock_repo: AsyncMock):
        types = [
    ...
    async def test_returns_empty_list_when_no_types(self, use_case: ListAllMeetingTypeUseCase, mock_repo: AsyncMock):
